...
public class GroceryAPI
{
    ...
    public async Task<IActionResult> CreateGrocery([HttpTrigger(AuthorizationLevel.Function, "post", Route = "GroceryList")] HttpRequest req)
    {
        ...
        string requestBody = await new StreamReader(req.Body).ReadToEndAsync(); //read json as string 
        GroceryItem_Upsert? data = JsonConvert.DeserializeObject<GroceryItem_Upsert>(requestBody); //deserilze json string as c# obj
